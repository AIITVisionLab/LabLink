      ...
      <section class="mobile-auth-card">
        ...
        >
          ...
          <div class="mobile-auth-section">
            <div class="mobile-auth-section-head">
              <strong>验证与说明</strong>
              <span>请使用常用邮箱接收验证码，并补充简要申请说明，方便学院和学校审核。</span>
            </div>
...
const sendEmailCode = async () => {
  const teacherNo = normalizedTeacherNo()
  const email = normalizedEmail()

  if (!teacherNoPattern.test(teacherNo)) {
    ElMessage.error('请先输入正确的工号')
    return
  }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
