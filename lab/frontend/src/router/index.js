...
import { isMobilePortal, resolvePortalHome, resolvePortalRole } from '@/utils/portal'
...
const routes = [
  ...
  },
  {
    ...
    children: [
      ...
      { path: 'labs', component: MobileStudentLabs, meta: { title: '实验室总览' } },
      ...
      { path: 'profile', component: MobileProfile, meta: { title: '我的' } }
    ]
  },
  {
    path: '/m/teacher',
    ...
    children: [
      ...
      { path: 'create-applies', component: MobileTeacherCreateApplies, meta: { title: '创建申请' } },
      { path: 'notices', component: MobileNotices, meta: { title: '公告中心' } },
      { path: 'profile', component: MobileProfile, meta: { title: '我的' } }
    ]
  },
...
router.beforeEach((to, from, next) => {
  ...
  const shouldUseMobile = isMobilePortal()

  if (shouldUseMobile) {
    if (to.path.startsWith('/student')) {
      next({ path: to.path.replace(/^\/student/, '/m/student'), query: to.query, hash: to.hash })
      return
    }
    if (to.path.startsWith('/teacher')) {
      next({ path: to.path.replace(/^\/teacher/, '/m/teacher'), query: to.query, hash: to.hash })
      return
    }
  } else {
    if (to.path.startsWith('/m/student')) {
      next({ path: to.path.replace(/^\/m\/student/, '/student'), query: to.query, hash: to.hash })
      return
    }
    if (to.path.startsWith('/m/teacher')) {
      next({ path: to.path.replace(/^\/m\/teacher/, '/teacher'), query: to.query, hash: to.hash })
      return
    }
  }
